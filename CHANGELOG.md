...
- **Breaking: a to-one onto a composite key declares a column per key and pairs each in `references`**, instead of deriving `<relation><Key>` columns.
- **Breaking (types): `references` and `mappedBy` only compile on columns that can hold the key they join**, and `mappedBy` names no method.
- **Breaking (types): what registration refuses no longer compiles**: a security filter that skips, a filter named `softDelete`, a column referencing a composite key.
- **An entity refuses a join on a member that is not a column, or on a foreign key to another entity**, on either side, on first read.
- **Fixed: a primary key referencing another entity is never generated by the database**, and `getSqlType` keeps a foreign key's own `columnType`.
- **Fixed: `@Entity({ relations })` types its callbacks against the class it decorates**; a `references` callback kept it from compiling.
