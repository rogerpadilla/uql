...
- **A member is never named by a string.** A statement is JSON, so it names one by a key (`$sum: { amount: true }`; one member is `QueryFieldRef<E>`); a definition is code, so it reads one off a map: `RefMap<E>` where it is SQL or an index (``@Index((post) => [post.title, raw`lower(${post.slug})`])``, `computed`, a check), whose refs render as columns and carry their key, and `KeyMap<E>` where it names a relation or a method (`mappedBy: (post) => post.author`, `hooks`). Only the member lists are callbacks: TypeScript never checks a callback's returned literal for excess properties, so the options around them stay a literal of their own. Names stay strings only where they are data: the migration builder's `table.index([...])` and the incremental `defineField`/`defineRelation`/`defineHook`. **Every map keyed by an entity's members is declared over a type parameter constrained to `keyof E`** (`QuerySelect<E, F extends keyof E = FieldKey<E>>`, the way `Pick` is), which keeps each key linked to its property for rename and find-references. A map over a computed key set (`[K in FieldKey<E>]`) type-checks the same and silently loses the link: `tsc` cannot tell, `bun run ts.rename` can.
