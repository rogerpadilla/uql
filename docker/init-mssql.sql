...
IF DB_ID('test_trigger') IS NULL BEGIN CREATE DATABASE test_trigger; ALTER DATABASE test_trigger SET READ_COMMITTED_SNAPSHOT ON; END
IF DB_ID('test_drift') IS NULL BEGIN CREATE DATABASE test_drift; ALTER DATABASE test_drift SET READ_COMMITTED_SNAPSHOT ON; END
IF DB_ID('test_stamp') IS NULL BEGIN CREATE DATABASE test_stamp; ALTER DATABASE test_stamp SET READ_COMMITTED_SNAPSHOT ON; END
IF DB_ID('test_better_auth') IS NULL BEGIN CREATE DATABASE test_better_auth; ALTER DATABASE test_better_auth SET READ_COMMITTED_SNAPSHOT ON; END
SELECT 1
