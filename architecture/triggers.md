...
- **SQL Server hands the body `inserted` and `deleted` as tables**, not records, so a comparison cannot read one row directly. `of` and `where` narrow the rows the write helpers read instead of guarding the statement, which would let every row through once one qualified; SQL of its own reads the tables whole, so it is refused beside them. `of` is `EXISTS (SELECT ... EXCEPT SELECT ...)`: null-safe, and comparing values rather than asking `UPDATE(col)`, which fires on a column merely assigned. The join is on the key, the only pairing SQL Server offers, so a statement changing a row's key leaves that row unpaired: its filter and its writes see nothing. Pairing a single-row statement regardless would need an `OR` in the join, a nested loop over every bulk update.
