...
Elsewhere, a census of unrelated codebases found only `updated_at` stamps and one `NOTIFY`, which is why stamps and authored triggers are here. Trigger libraries (django-pgtrigger, alembic_utils, Atlas, MikroORM 7.2) author and diff triggers but build no aggregate on them. Rails' `counter_culture` maintains counters in the application and ships `fix_counts` to repair them. No other tool offers a maintained aggregate that starts life as an ordinary read.
