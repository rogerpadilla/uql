...
**Stored triggers.** Authored triggers and `stored: ['update']` stamps shipped, on every SQL engine, needing neither R7 nor R7b: a trigger is recreated rather than diffed, and the engine keeps the render a rollback puts back. What is left is the maintained aggregate (`computed: (u) => u.resources.count(), stored: true`), held back until an unstored one profiles too slow. MongoDB has no triggers and refuses them. [The design](triggers.md).
