...
**Stored triggers** (R7, then R7b). Stored aggregates (`computed: (u) => u.resources.count(), stored: true`), then `stored: ['update']` stamps, then authored triggers. Postgres first, then a renderer per SQL engine; MongoDB has no triggers and refuses `stored`. [The design](triggers.md).
