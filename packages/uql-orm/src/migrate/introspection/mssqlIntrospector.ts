...
export class MsSqlSchemaIntrospector extends AbstractSqlSchemaIntrospector {
  ...
  protected async mapColumnsResult(
    ...
    // One extra statement for both flags, and the reader sends it once: a column is in the key or
    // uniquely indexed on its own, and `sys` is the only place either is readable per column.
    const flags = await read<{ column_name: string; is_primary_key: boolean }>(
      /*sql*/ `
      SELECT c.name as column_name, MAX(CAST(i.is_primary_key AS INT)) as is_primary_key
      FROM sys.indexes i
      JOIN sys.objects o ON o.object_id = i.object_id
      JOIN sys.schemas s ON s.schema_id = o.schema_id
      JOIN sys.index_columns ic ON ic.object_id = i.object_id AND ic.index_id = i.index_id
      ...
      WHERE s.name = ${this.schemaExpr} AND o.name = @p1
        ...
        AND (SELECT COUNT(*) FROM sys.index_columns k
             WHERE k.object_id = i.object_id AND k.index_id = i.index_id AND k.is_included_column = 0) = 1
      GROUP BY c.name
    `,
      [tableName],
    );
    const unique = new Set(flags.map((row) => row.column_name));
    const keys = new Set(flags.filter((row) => Boolean(row.is_primary_key)).map((row) => row.column_name));

    return results.map((row) => {
      const type = row.data_type.toUpperCase();
      const wide = type.startsWith('N');
      const bytes = this.toNumber(row.max_length);
      const length =
        bytes === undefined || bytes < 0 || !CHARACTER_TYPES.has(type) ? undefined : wide ? bytes / 2 : bytes;
      return {
        ...
        isPrimaryKey: keys.has(row.column_name),
        isUnique: unique.has(row.column_name),
        length,
