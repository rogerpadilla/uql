...
for (const db of databases) {
  describe(`Migrator autoSync Integration (${db.name})`, () => {
    ...
    it('should detect and sync a new property added to an existing entity', async () => {
      ...
      await givenTable(tableName, `${db.serialPrimaryKey}, ${escapeId('name')} ${db.textType}`);
    ...
    it('should create an index the entity declares on a table that already exists', async () => {
      ...
      await givenTable(tableName, `${db.serialPrimaryKey}, ${escapeId('email')} ${db.textType}`);
    ...
    it('should handle entity with custom table name', async () => {
      ...
      await givenTable(tableName, `${db.serialPrimaryKey}, ${escapeId('username')} ${db.textType}`);
    ...
    it('should handle field rename safely (add new, keep old)', async () => {
      ...
      await givenTable(tableName, `${db.serialPrimaryKey}, ${escapeId('oldName')} ${db.textType}`);
    ...
    it('should NOT alter existing DOUBLE column to BIGINT for number field (Safe Mode)', async () => {
      ...
      await givenTable(tableName, `${db.serialPrimaryKey}, ${escapeId('cost')} ${db.doubleType}`);
    ...
    it('should block drops even if safe: false (when drop: false)', async () => {
      ...
      await givenTable(
        tableName,
        `${db.serialPrimaryKey}, ${escapeId('name')} ${db.textType}, ${escapeId('extraColumn')} ${db.textType}`,
      );
