...
describe('entityPath', () => {
  it('should kebab-case the entity name', () => {
...
describe('matchRoute', () => {
  it('should match root routes by method', () => {
  ...
  it('should fall back to :id routes for non-literal sub-paths', () => {
  ...
  it('should map QUERY (RFC 10008) to the read operations only', () => {
  ...
  it('should be case-insensitive on the method', () => {
    expect(matchRoute('get', 'one')).toEqual({ op: 'findOne', method: 'GET' });
    expect(matchRoute('delete', undefined)).toEqual({ op: 'deleteMany', method: 'DELETE' });
  });

  it('should serve HEAD as GET per HTTP semantics', () => {
...
describe('toErrorResponse', () => {
  it('should map a plain Error to a 500 envelope', () => {
  ...
  it('should honor a numeric status on the error', () => {
  ...
  it.each<[string, Error, number, string]>([
    ...
    ['a payload with no version', new UqlUsageError('carries no version'), 400, 'carries no version'],
    [
      'a $lock the engine lacks',
      new UqlUsageError('sqlite does not support row-level locking'),
      400,
  ...
  it('should map non-Error values to a generic 500', () => {
