...
describe('SqlSchemaGenerator (Postgres)', () => {
  ...
  it('should generate CREATE TABLE for simple entity', () => {
    ...
    // A unique column is a unique index, beside the table rather than a constraint inside it.
    expect(sql).toContain('"email" TEXT,');
    expect(sql).toContain('CREATE UNIQUE INDEX "TestUser__email_idx" ON "TestUser" ("email");');
  ...
  it('should optimize a MySQL-family table a fulltext index is added to, and no other', () => {
    const diff: SchemaDiff = {
      ...
      type: 'alter',
      indexes: [
        { to: { name: 'docs_title_idx', entries: [{ column: 'title' }], unique: false, type: 'fulltext' } },
        { to: { name: 'docs_slug_idx', entries: [{ column: 'slug' }], unique: false } },
      ],
    };
  ...
  it('should emit CREATE VECTOR INDEX for MariaDB', () => {
    ...
    expect(statements.at(-1)).toBe('CREATE VECTOR INDEX `vec_idx` ON `embeddings` (`embedding`) DISTANCE=cosine;');
    expect(statements[0]).not.toContain('VECTOR INDEX');
    expect(statements.join('\n')).not.toContain('CREATE EXTENSION');
  });

  /**
   * Verified against MariaDB 12.3: a nullable column makes the whole statement fail with "All parts of
   * a VECTOR index must be NOT NULL". The entity's schema says so, so drift and a diff agree with it.
   */
  it('should make a MariaDB vector-indexed column NOT NULL, whatever the entity declared', () => {
    @Index((doc) => [doc.embedding], { type: 'vector', distance: 'cosine' })
    @Entity()
    class NullableEmbedding {
      @Id({ type: Number }) id?: number;
      @Field({ type: 'vector', dimensions: 3 }) embedding?: number[] | null;
    }
    const nullable = (dialect: MariaDialect | PostgresDialect) =>
      new SqlSchemaGenerator(dialect)
        .buildAST([NullableEmbedding])
        .getTable('NullableEmbedding')
        ?.columns.get('embedding')?.nullable;

    expect(nullable(new MariaDialect())).toBe(false);
    expect(nullable(new PostgresDialect())).toBe(true);
  });
  ...
  it('should generate ALTER TABLE DOWN statements for index additions', () => {
    const sql = generator.generateAlterTable(
      reverseDiff({
        tableName: 'users',
        type: 'alter',
        indexes: [{ to: { name: 'users__email_idx', entries: [{ column: 'email' }], unique: true } }],
      }),
    );
    expect(sql[0]).toBe('DROP INDEX IF EXISTS "users__email_idx";');
  });

  /** A drop keeps the whole column, so its rollback adds it back, empty. */
  it('should restore a dropped column on the way down', () => {
    const diff: SchemaDiff = { tableName: 'users', type: 'alter', columns: [{ from: namedColumn('old_column') }] };
  ...
  it('should drop an index before recreating one under its name, and reverse both on the way down', () => {
    const diff: SchemaDiff = {
      ...
      type: 'alter',
      indexes: [
        { from: { name: 'lookup', entries: [{ column: 'name' }], unique: false } },
        { to: { name: 'lookup', entries: [{ column: 'name' }, { column: 'email' }], unique: false } },
      ],
    };
  ...
  it('should generate ALTER TABLE statements for dropping columns and indexes', () => {
    const sql = generator.generateAlterTable({
      tableName: 'users',
      type: 'alter',
      columns: [{ from: namedColumn('old_col') }],
      indexes: [
        { from: { name: 'old_idx', entries: [{ column: 'old_col' }], unique: false } },
        { to: { name: 'new_idx', entries: [{ column: 'name' }], unique: false } },
      ],
    });
    expect(sql).toEqual([
...
describe('SqlSchemaGenerator column definitions from ColumnSchema', () => {
  ...
  /** A unique column is a unique index, which the table creates beside it. */
  it("should leave a unique column's uniqueness to its index", () => {
    expect(addColumn({ isUnique: true })).toBe('ALTER TABLE "users" ADD COLUMN "col" INTEGER;');
  });
  ...
  it('should strip PRIMARY KEY when restoring a column in the down direction', () => {
    const sql = new SqlSchemaGenerator(new MySqlDialect()).generateAlterTable(
      reverseDiff({
        tableName: 'users',
        type: 'alter',
        columns: [
...
describe('SqlSchemaGenerator diffs (Postgres)', () => {
  ...
  it('should reverse a primary key change that added a key, or removed one', () => {
    const added = generator.generateAlterTable(
      reverseDiff({
        tableName: 'users',
        type: 'alter',
        primaryKey: { to: { columns: ['id'], name: 'users__id_pk' } },
      }),
    );
    expect(added).toEqual([expect.stringContaining('DROP CONSTRAINT')]);

    const removed = generator.generateAlterTable(
      reverseDiff({
        tableName: 'users',
        type: 'alter',
        primaryKey: { from: { columns: ['id'], name: 'users_pkey' } },
      }),
    );
    expect(removed).toEqual([expect.stringContaining('ADD CONSTRAINT')]);
  });

  /** A key nothing named gets the name this generator derives, the one a rollback then drops it by. */
  it('should add a key under the derived name where the change names none', () => {
    const diff: SchemaDiff = { tableName: 'users', type: 'alter', primaryKey: { to: { columns: ['id'] } } };
    expect(generator.generateAlterTable(diff)).toEqual([
      'ALTER TABLE "users" ADD CONSTRAINT "users__id_pk" PRIMARY KEY ("id");',
    ]);
  });
  ...
  it('should detect new columns', () => {
    ...
    expect(added(diff?.columns)).toHaveLength(2);
    expect(added(diff?.columns)[0].name).toBe('email');
    expect(added(diff?.columns)[1].name).toBe('status');
  });
  ...
  it('should detect columns to drop', () => {
    ...
    expect(diff).toBeDefined();
    expect(diff?.type).toBe('alter');
    expect(dropped(diff?.columns).map((column) => column.name)).toEqual(['old_col']);
  });
  ...
  it('should add an index the entity declares and the table has not got', () => {
    ...
    expect(added(diff?.columns)).toEqual([]);
    expect(added(diff?.indexes).map((index) => index.name)).toEqual(['DiffUser__status_idx']);
  });
  ...
  it('should rebuild an index whose uniqueness differs', () => {
    const currentSchema = mockSqlTableNode('DiffUser', [
      { name: 'id', sql: 'INTEGER', isPrimaryKey: true, isAutoIncrement: true },
      { name: 'name', sql: 'VARCHAR', length: 255 },
      { name: 'email', sql: 'VARCHAR', length: 100 },
    ...
    expect(added(diff.indexes)).toEqual([]);
    expect(generator.generateAlterTable(diff)).toEqual([
      'DROP INDEX IF EXISTS "DiffUser__status_idx";',
      'CREATE INDEX IF NOT EXISTS "DiffUser__status_idx" ON "DiffUser" ("status");',
    ]);
    expect(generator.generateAlterTable(reverseDiff(diff))).toEqual([
      'DROP INDEX IF EXISTS "DiffUser__status_idx";',
      'CREATE UNIQUE INDEX IF NOT EXISTS "DiffUser__status_idx" ON "DiffUser" ("status");',
    ]);
  });
  ...
  it('should read a reported null default and an absent one as the same', () => {
    expect(generator.defaultsEqual(undefined, null)).toBe(true);
    expect(generator.defaultsEqual(null, undefined)).toBe(true);
    expect(generator.defaultsEqual(null, null)).toBe(true);
    expect(generator.defaultsEqual('active', null)).toBe(false);
  ...
  it('should produce the SQL of an alter', () => {
    const sql = generator.generateAlterTable({
      ...
      columns: [
        ...
        { from: namedColumn('old_name') },
      ],
      indexes: [
        { to: { name: 'age_idx', entries: [{ column: 'age' }], unique: false } },
        { from: { name: 'old_idx', entries: [{ column: 'old_name' }], unique: false } },
      ],
    });
...
/** A plain nullable text column named `name`, the whole column a dropped one keeps. */
function namedColumn(name: string): ColumnSchema {
  return { name, type: 'TEXT', nullable: true, isPrimaryKey: false, isAutoIncrement: false, isUnique: false };
}
