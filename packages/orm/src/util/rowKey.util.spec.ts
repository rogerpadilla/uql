...
describe('rowKey', () => {
  it('should key a single column by its value', () => {
    expect(rowKey({ id: 1 }, ['id'])).toBe('1');
  });

  it('should key every column, so rows agreeing on one column of a composite differ', () => {
    const columns = ['a', 'b'];
  ...
  it('should do not collide across a separator a value could contain', () => {
    const columns = ['a', 'b'];
  ...
  it('should key equal dates the same', () => {
    expect(rowKey({ at: new Date('2026-01-01T00:00:00Z') }, ['at'])).toBe('2026-01-01T00:00:00.000Z');
  });

  it('should key bytes by their hex, which commas in a stringified array would blur', () => {
  ...
  it('should key no columns as the empty string', () => {
    expect(rowKey({}, [])).toBe('');
  });

  it('should read a column the row does not carry as undefined rather than throwing', () => {
